...
public class Indexer
{
    ...
    private Document getHTMLDocument(String path, int id)
    {
        ...
        */
        ...
        // [DONE] create a field that is stored but not indexed
        // and contains document's id
        // ----------------------------------
        StoredField idField = new StoredField(Constants.id, id);
        // ----------------------------------

        // [DONE] create a field that is indexed but not stored
        ...
        // ----------------------------------
        String textFromHTMLFile = getTextFromHTMLFile(file);
        if(textFromHTMLFile == null) {
            textFromHTMLFile = "";
        }
        TextField contentField = new TextField(Constants.content, textFromHTMLFile, Field.Store.NO);
        // ----------------------------------

        // [DONE] create a field that is stored and indexed
        // and contains file name
        // ----------------------------------
        TextField fileNameField = new TextField(Constants.filename, file.getName(), Field.Store.YES);
        // ----------------------------------

        // [DONE] create an INT field (IntPoint) that is indexed
        // and contains file size (bytes, .length())
        // ----------------------------------
        IntPoint intField = new IntPoint(Constants.filesize_int, (int)file.length());

        // ----------------------------------
        // [DONE] IntPoint is not stored but we want to a file size
        // ... so add another field (StoredField) that stores file size
        // ----------------------------------
        StoredField storedSizeField = new StoredField(Constants.filesize, file.length());
        // ----------------------------------

        // [DONE] add fields to the document object
        // ----------------------------------
